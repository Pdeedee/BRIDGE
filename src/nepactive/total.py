...
if __name__ == "__main__":
    try:
        ...
        print("检测到中断，清理子进程...")
        os._exit(1)  # 强制退出主进程（避免僵尸进程）__version__ = "0.0.1"
...
def parse_yaml(file):
    ...
    return datafrom concurrent.futures import ProcessPoolExecutor
import torch
from mattersim.forcefield import MatterSimCalculator
from ase.io import read, write
from ase import Atoms
...
class NPHugo(MTTK):
    ...
    def step(self):
        ...
        self.second_half()
        self._center_of_velocities
        print(f"dhugo:{self.dhugo}")from __future__ import annotations
...
class NPHugo(MTTK):
    ...
    def step(self):
        ...
        self.second_half()
        self._center_of_velocities
        print(f"dhugo:{self.dhugo}")from nepactive.nphugo import MTTK, NPHugo
...
if __name__ == "__main__":
    ...
    result_solution, result_error = solver.solve_difficult_case(c, h, o, n)import numpy as np
from scipy.optimize import linprog, milp
import random
import time
...
if __name__ == "__main__":
    ...
    print(f"误差: {result['error']}")from nepactive.nphugo import MTTK, NPHugo
# from nepactive.random_stable import solve_molecular_distribution
...
class StableRun:
    ...
    def calculate_properties(self):
        ...
        if os.path.exists("properties.txt"):
            ...
            os.chdir("structure")
            if not os.path.exists("task_finished"):
                nvt_pyfile = nvt_pytemplate.format(structure=self.struc_file_name,temperature = 300, steps = 5000)
    ...
    def run_pytasks(self):
        ...
        task_per_gpu = self.sdata.get("task_per_gpu",4)
        gpu_available = self.sdata.get("gpu_available",[0,1,2,3])
        tasknum_pertime = task_per_gpu * len(gpu_available)
    ...
    def make_pytasks(self):
        ...
        for ii,_ in enumerate(self.pressure_list):
            ...
            if ensemble == "nphugo":
                ...
                py_file = nvt_pytemplate.format(structure = "../structure/POSCAR",temperature = 300, dump_freq = dump_freq, steps = steps)
            elif ensemble == "npt":
                py_file = npt_pytemplate.format(structure = "../structure/POSCAR",temperature = 300, pressure=self.pressure_list[ii], 
    ...
    def run_gpumd_tasks(self):
        gpu_available = self.sdata.get("gpu_available",[0,1,2,3])
        self.task_per_gpu = self.sdata.get("task_per_gpu",1)
        run_gpumd_task(gpu_available=gpu_available, task_per_gpu=self.task_per_gpu)
...
def get_shortest_distance(atoms:Atoms,atom_index=None):
    ...
    return np.min(distance_matrix)import os
from nepactive import dlog
from nepactive.remote import Remotetask
import shutil
import subprocess
...
class Nepactive_OB(Nepactive):
    ...
    #     np.savetxt(f"{self.work_dir}/iter.{self.ii:06d}/steps.txt",np.array([self.run_steps]),fmt="%12d")import os
from nepactive import dlog
from nepactive.remote import Remotetask
import shutil
import subprocess
...
class Nepactive(object):
    def __init__(self,idata:dict):
        ...
        if self.shock_run:
            if os.path.exists(f"{self.work_dir}/nostrucnum"):
                ...
                self.idata["structure_files"] = ["POSCAR"]
                self.idata["model_devi_general"][0]["structure_id"] = [[0]]
            else:
    ...
    def make_init_ase_run(self):
        ...
        dlog.info(f"Found {len(task_dirs)} task directories to run.")
        for index,task_dir in enumerate(task_dirs):
            os.chdir(task_dir)
            if os.path.exists("task_finished"):
                atoms = read("out.traj", index=-1)
...
def _read_xyz_frame(lines, natoms, properties_parser=key_val_str_to_dict,
    ...
    # Fix old 'force' property name
    if 'force' in info['Properties'] and 'forces' not in info['Properties']:
        info['Properties'] = info['Properties'].replace('force','forces',1)
...
def set_calc_and_arrays(atoms, arrays):
    ...
    per_config_properties.append('virial')
    for key in list(atoms.info):
        if key in per_config_properties:
            ...
            if key == 'virial' and results[key].shape == (3, 3):
                stress = (results[key] / atoms.get_volume()) # set stress to ase style
                stress = np.array([stress[0, 0],
                                   stress[1, 1],
                                   stress[2, 2],
                                   stress[1, 2],
    ...
    for atoms in images:
        ...
        if write_results:
            ...
            if atoms.info.get('stress', np.array([])).shape == (6,):
                atoms.info['stress'] = \
                     voigt_6_to_full_3x3_stress(atoms.info['stress']) #change!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        for column, ncol in zip(fr_cols, ncols):
            ...
            if ncol == 1:
                ...
                for c in range(ncol):
                    ...
                    # except:
                    #     print(f'Error writing column "{column}"')
...
def save_calc_results(atoms, calc=None, calc_prefix=None,
    ...
    # print(f"atoms.info1008{atoms.info}")
    atoms.arrays.update(per_atom_results)
    # print(f"atoms.info1011{atoms.info}")
